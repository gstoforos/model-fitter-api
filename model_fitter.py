...
def fit_newtonian(gamma_dot, sigma):
    ...
    try:
        ...
        return {'model': 'Newtonian', 'sigma0': 1e-6, 'k': 1e-6, 'n': 1e-6, 'mu': None, 'r2': 0.0}
...
def fit_power_law(gamma_dot, sigma):
    ...
    try:
        ...
        return {'model': 'Power Law', 'sigma0': 1e-6, 'k': 1e-6, 'n': 1e-6, 'mu': None, 'r2': 0.0}

# Herschel–Bulkley: σ = σ₀ + K * γ̇ⁿ
def fit_herschel_bulkley(gamma_dot, sigma):
    ...
    try:
        ...
        return {'model': 'Herschel–Bulkley', 'sigma0': 1e-6, 'k': 1e-6, 'n': 1e-6, 'mu': None, 'r2': 0.0}
...
def fit_all_models(gamma_dot, sigma):
    ...
    # Selection logic
    if all(m['r2'] > 0.99 for m in valid_models):
        return next(m for m in valid_models if m['model'] == 'Newtonian')

    power = next((m for m in valid_models if m['model'] == 'Power Law'), None)
    hb = next((m for m in valid_models if m['model'] == 'Herschel–Bulkley'), None)
    bingham = next((m for m in valid_models if m['model'] == 'Bingham Plastic'), None)
